    ...
    switch (key) {
        ...
        case "state": {
            if (typeof value !== "object" || value == null) {
                throw new Error(`Expected an object (state), but got ${util_1.default.inspect(value)}`);
            }
            if (!hasOwnProperty(value, "softErrors") ||
                typeof value.softErrors !== "boolean") {
                throw new Error(`Expected a boolean (messages.softErrors), but got ${util_1.default.inspect(value.softErrors)}`);
            }
            const stateConfig = {
...
function validateEnvironmentOverrides(environment) {
    ...
    if (hasOwnProperty(environment, "stateSoftErrors")) {
        ...
        if ((0, type_guards_1.isBoolean)(stateSoftErrors)) {
            overrides.stateSoftErrors = stateSoftErrors;
        }
        else if ((0, type_guards_1.isString)(stateSoftErrors)) {
            overrides.stateSoftErrors = stringToMaybeBoolean(stateSoftErrors);
        }
        else {
            throw new Error(`Expected a boolean (stateSoftErrors), but got ${util_1.default.inspect(stateSoftErrors)}`);
        }
    }
